Shader "VTS/GenericShader" {
    ...
    SubShader {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o) {
            ...
            const float ms     =(  ms0.r +   ms0.g*2 +   ms0.b*4 +   ms1.r*8 +   ms1.g*16 +   ms1.b*32)*1000.0/64 + (_IsPanorama ? _Time.y*1000.0 : 0);
            const float sec    =  sec0.r +  sec0.g*2 +  sec0.b*4 +  sec1.r*8 +  sec1.g*16 +  sec1.b*32 + ms/1000.0;
            const float minute =  min0.r +  min0.g*2 +  min0.b*4 +  min1.r*8 +  min1.g*16 +  min1.b*32 + sec/60.0;
            const float hour   = hour0.r + hour0.g*2 + hour0.b*4 + hour1.r*8 + hour1.g*16 + hour1.b*32 + minute/60.0;

            const int date   = date0.r + date0.g*2 + date0.b*4 + date1.r*8 + date1.g*16 + date1.b*32 + 1 + hour/24.0;

            // TODO: The following metrics will be inaccurate if an user spends several days in a world
            const int month  =month0.r +month0.g*2 +month0.b*4 +month1.r*8 +month1.g*16 +month1.b*32 + 1 + date/30.0;
